...
class IdempotencyConcurrencyTest {
    ...
    void singlePacketDeliveredByThreeBridgesSettlesExactlyOnce() throws Exception {
        ...
        BigDecimal aliceBefore = accounts.findById("tanish@demo").orElseThrow().getBalance();
        BigDecimal bobBefore = accounts.findById("rahul@demo").orElseThrow().getBalance();

        // One packet, but we'll deliver it from 3 "bridges" simultaneously
        MeshPacket packet = demoService.createPacket(
                "tanish@demo", "rahul@demo", new BigDecimal("100.00"), "1234", 5);
        ...
        BigDecimal aliceAfter = accounts.findById("tanish@demo").orElseThrow().getBalance();
        BigDecimal bobAfter = accounts.findById("rahul@demo").orElseThrow().getBalance();
    ...
    void tamperedCiphertextIsRejected() throws Exception {
        MeshPacket packet = demoService.createPacket(
                "tanish@demo", "rahul@demo", new BigDecimal("50.00"), "1234", 5);
    ...
    void encryptDecryptRoundTrip() throws Exception {
        PaymentInstruction original = new PaymentInstruction(
                "tanish@demo", "rahul@demo", new BigDecimal("123.45"),
